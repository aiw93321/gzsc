...
class AppLogService
{
    ...
    public static function addErrorLog($appname,$content)
    {
        ...
        $models_app_log = new AppLog();
        $models_app_log->app_name=$appname;
        $models_app_log->content=$content;
        //获取ip
        $models_app_log->ip = UtilService::getIP();
        if (!empty($_SERVER['HTTP_USER_AGENT'])){
            $models_app_log->ua = $_SERVER['HTTP_USER_AGENT'];
        }

        if ($error){
            $models_app_log->err_code = $error->getCode();
            if (isset($error->statusCode)){
                $models_app_log->http_code = $error->statusCode;
            }

            if (method_exists($error,'getName')){
                $models_app_log->err_name = $error->getName();
            }
        }

        $models_app_log->created_time = date('Y-m-d H:i:s');
        $models_app_log->save();
    }
